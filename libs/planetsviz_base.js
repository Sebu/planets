...
PModel = function(params) {
    ...
    this.metonZodicalMonths = function() {
        return this.metonYear + this.metonSynodicMonths;
    }
    this.metonDaysPerYear = function() {
        return this.metonDays / this.metonYear;
    }
    this.synodicDaysPerMonth = function() {
        return this.metonDays / this.metonSynodicMonths;
    }
    this.zodicalDaysPerMonth = function() {
        return this.metonDays / this.metonZodicalMonths();
    }
    this.draconiticDaysPerMonth = function() {
    ...
    this.camera.addNode(this.sphere[0] = new Spherical({scale: 9, angle: 0.0, speed: 0.0, color: colors["S0"]}));
    //	,
    ...
    for (; i <= params.spheres; i++) {
        tmp = this.sphere[i] = new Spherical({scale: 9, angle: 0.0, speed: 0.0, color: colors["S" + i + ""]});
        this.sphere[i - 1]._anchor2.addNode(tmp);
        this.updateList[i] = tmp;
    }
    this.sphere[i - 1]._anchor2.addNode(this.planet = new Planet({ dist: 9.0, emit: 0.5, scale: 0.2, inner_id: "planet",  color:colors["Planet"] }));
    ...
    this.sphere[0]._anchor.addNode(this.systemSun[0] = new Spherical({ scale: 9, angle: 24.0, speed: 365.0, color: {r:0.2, g:0.2, b:1.0}},
        ...
        if (this.time > 20) {
            ...
            if (this.showPath) {
                if (this.spheres == 1) {
                    ...
                    this.addCurve(0, this.sphere[0]._anchor, this.calcCurve(0, "planet"), colors["Path"]);
                }
            }
            if (this.showHippo)     this.addCurve(1, this.sphere[1]._anchor, this.calcCurve(1, "planet"), colors["Hippo"]);
        }
    ...
    this.reset = function () {
        ...
        for (var i = 0; i < this.sphere.length; i++) {
            this.sphere[i].setRotate(model.currentPlanet.sphere[i].rotate);
        }
    ...
    this.calcCurve = function(start, node) {
        ...
        for (var i = 0; i <= start; i++) {
            oldAngle[i] = this.sphere[i].getAngle();
            oldRotate[i] = this.sphere[i]._yAngle;
            this.sphere[i].setAngle(0.0);
            this.sphere[i].setRotate(0.0);
        }

        for (var i = start + 1; i < this.sphere.length; i++) {
            oldRotate[i] = this.sphere[i]._yAngle;
            //    system[i].update(-32.0);
            step += Math.abs(this.sphere[i]._step);
        }
        ...
        // restore axis
        for (var i = 0; i <= start; i++)
            this.sphere[i].setAngle(oldAngle[i]);

        // restore rotation
        for (var i = 0; i < this.sphere.length; i++)
            this.sphere[i].setRotate(oldRotate[i]);
